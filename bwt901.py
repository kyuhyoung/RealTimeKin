...
def serial_read(senser, kueue):
    while 1:
        rx_ry_rz = senser.getAngle()
        kueue.put(rx_ry_rz)
...
if __name__ == "__main__":
    ...
    li_com = ['COM4', 'COM5', 'COM11']
    #li_com = ['COM4']
    #li_com = ['COM11']

    li_sensor = []
    for idx, kom in enumerate(li_com):
        li_sensor.append(BWT901(kom))
    kount = 0

    while True:
        li_return = []
        for senser in li_sensor:
            li_return.append(senser.getAngle())
        if 0 == kount % 1:
            print('\nkount :', kount)
            for idx, kom in enumerate(li_com):
                print(kom, ':', li_return[idx])
        kount += 1
    ...
    li_queue = []
    for idx, kom in enumerate(li_com):
        li_queue.append(queue.Queue(2))
        senser = BWT901(kom)
        threading.Thread(target=serial_read, args=(senser, li_queue[idx],),).start()
    while True:
        li_return = []
        for kueue in li_queue:
            li_return.append(kueue.get(True, 1))
        if 0 == kount % 10:
            print('\nkount :', kount)
            for idx, kom in enumerate(li_com):
                print(kom, ':', li_return[idx])
